...
## 2026-02-10（P2：阅读页状态栏与顶部菜单对比度优化）

### 已完成
- `reader_status_bar.dart`：
  - 页眉/页脚分割线颜色改为亮暗分层语义（提升亮色主题下可见性）；
  - 状态文本透明度按亮暗模式区分（亮色主题提高可读性，暗色保持克制）；
  - 电池颜色语义统一到 `AppDesignTokens`（低电/中电/正常）。
- `reader_menus.dart`：
  - 顶部菜单“净化”等芯片激活态强调色改为品牌语义色（亮色主品牌、暗色副品牌），与阅读器其它控制面板一致；
  - 保持原有入口与操作顺序不变，仅优化视觉反馈一致性。

### 为什么
- 在亮色阅读主题下，页眉页脚与顶部菜单部分文本/分割线过淡，影响可读性和状态识别。
- 本次补齐“主阅读层 + 二级面板 + 状态条”三者之间的统一语义色系统，进一步增强整体协调。

### 验证方式
- 命令：`dart format lib/features/reader/widgets/reader_status_bar.dart lib/features/reader/widgets/reader_menus.dart`
- 命令：`flutter analyze --no-pub`
- 命令：`flutter test test/rule_parser_engine_css_nth_compat_test.dart`
