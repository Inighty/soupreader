...
/// 「导入候选 → 选择 → 提交」共享的内部入口。
///
/// 失败时由调用方再做提示；这里只把「确认导入选项」流程封装好。
Future<void> importTxtTocRulesFromInput({
  required BuildContext context,
  required TxtTocRuleStore ruleStore,
  ...
  if (!context.mounted) return;
  final selectedIndexes = await showTxtTocRuleImportSelectionSheet(
    context: context,
    candidates: candidates,
  );
  if (selectedIndexes == null || selectedIndexes.isEmpty) return;
  if (!context.mounted) return;
  await runTxtTocRuleImportingTask(
    context: context,
...
Future<String?> pickTxtTocRuleLocalImportText() async {
...
bool isTxtTocRuleHttpUrl(String value) {
...
Future<void> importTxtTocRulesFromQr({
  required BuildContext context,
  required TxtTocRuleStore ruleStore,
}) async {
