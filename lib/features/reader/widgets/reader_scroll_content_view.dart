...
@immutable
class ReaderScrollContentConfig {
  const ReaderScrollContentConfig({
  ...
  bool operator ==(Object other) {
    ...
    if (other is! ReaderScrollContentConfig) return false;
...
class ReaderScrollContentView extends StatefulWidget {
  ...
  final ReaderScrollContentConfig config;
  ...
  @override
  State<ReaderScrollContentView> createState() => ReaderScrollContentViewState();
}
class ReaderScrollContentViewState extends State<ReaderScrollContentView> {
  ReaderScrollContentConfig? _lastConfig;
  EdgeInsets? _lastScrollInsets;
  Widget? _cachedContent;
  @override
  void didUpdateWidget(ReaderScrollContentView oldWidget) {
  ...
  Widget _buildSegmentBody(ScrollSegment segment, {required bool isTailSegment}) {
  ...
  List<ReaderRenderBlock>? _buildImageRenderBlocks(String content) {
    final imageStyle = widget.config.imageStyle;
    if (imageStyle == ReaderImageStyles.text ||
        !ReaderImageStyles.imgTagRegex.hasMatch(content)) {
      return null;
    }
    final blocks = <ReaderRenderBlock>[];
    var cursor = 0;
    for (final match
        in ReaderImageStyles.imgTagRegex.allMatches(content)) {
    ...
    if (cursor < content.length) {
      ...
      if (trailing.trim().isNotEmpty) {
        blocks.add(ReaderRenderBlock.text(trailing));
      }
    }
  ...
  Widget _buildImageBlock({required String src, required double bodyWidth}) {
    ...
    final forceFullWidth =
        cfg.imageStyle == ReaderImageStyles.full ||
            cfg.imageStyle == ReaderImageStyles.single;
    final image = Image(
      image: imageProvider,
    ...
    if (cfg.imageStyle == ReaderImageStyles.single) {
