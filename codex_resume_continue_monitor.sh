#!/usr/bin/env bash
#
# codex_resume_continue_monitor.sh — resume a specific Codex conversation id
# and continuously send prompt "continue" in a loop.
# If a run stays silent for too long, it is terminated and restarted.
# Optional no-op worktree detection is preserved but disabled by default.
...
usage() {
  cat <<'EOF'
用法：
  ./codex_resume_continue_monitor.sh <会话ID>

说明：
  - 会话 ID 也可通过环境变量 CODEX_MONITOR_SESSION_ID 提供。
  - 每轮执行：codex exec resume <会话ID> "continue"
  - 默认无限循环运行。
EOF
}
...
if (( $# > 1 )); then
  echo "[codex-monitor] 参数过多。" >&2
  usage >&2
  exit 2
fi

session_id="${1:-${CODEX_MONITOR_SESSION_ID:-}}"
if [[ -z "$session_id" ]]; then
  echo "[codex-monitor] 缺少会话 ID。" >&2
  usage >&2
  exit 2
fi

continue_prompt="${CODEX_MONITOR_CONTINUE_PROMPT:-continue}"
...
pause_seconds=${CODEX_MONITOR_INTERVAL_SECONDS:-5}
require_worktree_change=${CODEX_MONITOR_REQUIRE_WORKTREE_CHANGE:-0}
