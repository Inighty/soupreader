...
void main() {
  testWidgets('书源调试书籍源码入口在源码为空时也直接打开源码承载页', (WidgetTester tester) async {
    ...
    final bookSrcAction = find.text('书籍源码');
    expect(bookSrcAction, findsOneWidget);
    await tester.tap(bookSrcAction);
    await tester.pumpAndSettle();

    expect(find.byType(SourceDebugTextView), findsOneWidget);
    expect(find.text('html'), findsOneWidget);
    expect(find.text('暂无书籍源码'), findsNothing);
  });
}
