...
void main() {
  testWidgets('书源调试搜索源码入口在源码为空时也直接打开源码承载页', (WidgetTester tester) async {
    ...
    final searchSrcAction = find.text('搜索源码');
    expect(searchSrcAction, findsOneWidget);
    await tester.tap(searchSrcAction);
    await tester.pumpAndSettle();

    expect(find.byType(SourceDebugTextView), findsOneWidget);
    expect(find.text('html'), findsOneWidget);
    expect(find.text('暂无搜索源码'), findsNothing);
  });
}
