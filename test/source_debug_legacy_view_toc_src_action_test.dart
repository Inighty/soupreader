...
void main() {
  testWidgets('书源调试目录源码入口在源码为空时也直接打开源码承载页', (WidgetTester tester) async {
    ...
    final tocSrcAction = find.text('目录源码');
    expect(tocSrcAction, findsOneWidget);
    await tester.tap(tocSrcAction);
    await tester.pumpAndSettle();

    expect(find.byType(SourceDebugTextView), findsOneWidget);
    expect(find.text('html'), findsOneWidget);
    expect(find.text('暂无目录源码'), findsNothing);
  });
}
