...
- **默认开发顺序为“核心功能优先、扩展功能后置”；未获得需求方“开始做扩展功能”的明确指令前，禁止启动扩展功能开发。**
...
- 涉及页面迁移或 UI 调整时，排版结构、信息层级、间距节奏、对齐方式必须与 legado 语义同义（平台组件差异除外）。
- 同一功能在不同状态（加载、空态、错误、成功）下的布局规则必须一致，禁止出现局部页面“单状态走样”。
- 涉及列表、详情、工具栏、弹窗等高频区域时，需逐项核对布局与交互热区，不得以“主流程可用”替代排版一致性验收。
- 若存在暂无法等价的布局能力，必须按第 `1.1.2` 章流程先标记 `blocked` 并完成确认记录后再继续。

### 1.3 核心优先与扩展冻结（强制）

- 核心功能：指直接影响搜索（search）、发现（explore）、详情（bookInfo）、目录（toc）、正文（content）可用性的能力及其依赖链路。
- 扩展功能：指不影响上述主链路可用性的增强项（附加入口、附加设置、附加样式、实验能力等）。
- 默认执行策略：未收到需求方“开始做扩展功能”的明确指令前，仅可推进核心功能；扩展项必须在计划中标记为 `blocked`。
- 解锁条件：仅当需求方明确下达“开始做扩展功能”指令后，才可将扩展项转为 `active` 并实施。
- 禁止事项：不得以“顺手优化”“预埋接口”“先占坑”名义提前实现扩展入口、扩展流程、扩展文案或扩展开关。
...
- 验收结果必须包含可复现证据：
  - 命令验证（如 `flutter analyze`）
  - 手工回归路径（入口与操作步骤）
- 在逐项对照完成前，不得直接宣称“已经一致”。

### 2.4 逐项检查与防遗漏（强制）

- 每个迁移/改造任务在实现前，必须先给出“逐项检查清单”，至少包含：入口、状态、异常、文案、排版、交互触发。
- 实现完成后必须按清单逐项核验并记录结果；存在未检查项时，任务状态不得标记为完成。
- 结论输出必须明确列出已检查项与未通过项处理结果，禁止仅用“已检查”“基本一致”等笼统表述替代。
...
- 中途需要校验时，仅使用与改动相关的手工回归路径；完整 `flutter analyze` 保留到提交前。
- 若任务为迁移级别开发，除上述硬性检查外，还必须补充与改动相关的手工回归验证，并写入对应 ExecPlan 文档。
...
- 每次完成一个可交付点（修复/新增/调整）后，**必须**同步更新：
  ...
  - 快速上下文文件：`docs/plans/ACTIVE_CONTEXT.md`（用于快速定位下一任务，避免全量重读计划）
- 更新内容至少包含：
  - 做了什么（变更点）
  - 为什么（问题/需求）
  - 如何验证（`flutter analyze` 或手工路径）
  - 当前任务与下一任务（含状态）
  - 阻塞项（如有）
...
- 若同一需求同时包含核心与扩展项，必须先串行完成核心项并验收；在收到需求方“开始做扩展功能”前，扩展项必须保持 `blocked`，且不得并行启动。
...
- 所有 UI 相关 Todo 必须包含“排版布局一致性”检查子项，并在交付时逐项回填核验结果。
...
- 目标：减少每次执行前对超大计划文件的全量扫描成本，改为“轻量快照优先、按需深读”。
- 统一快速入口文件：`docs/plans/ACTIVE_CONTEXT.md`（任务级运行态单一入口）。
- 默认读取顺序（未触发例外前必须遵守）：
  1. 完整读取 `docs/plans/ACTIVE_CONTEXT.md`；
  2. 仅读取 `PLANS.md` 前置索引区（建议前 `120` 行）确认主计划/Todo；
  3. 仅按 `ACTIVE_CONTEXT.md` 提供的目标章节，定向读取对应 ExecPlan 小节（如 `Progress` 中当前项），禁止默认全量读取所有计划文件。
- 仅在以下场景允许升级为“深度读取”（可扩大到完整 ExecPlan/台账）：
  - `ACTIVE_CONTEXT.md` 与 `PLANS.md`/ExecPlan 关键信息不一致；
  - 当前任务状态从 `active` 进入 `blocked`，或出现迁移例外（见第 `1.1.2` 章）；
  - 需要切换 Phase/里程碑，或无法从快照判定下一任务；
  - 需求方明确要求全量审阅计划历史。
- `ACTIVE_CONTEXT.md` 维护要求（每个可交付点必更）：
  - 必填：`最后更新时间`、`当前任务`、`下一任务`、`阻塞项`、`最近交付`；
  - 内容应保持轻量（建议不超过 `220` 行，仅保留最近 `5` 条交付摘要）；
  - 该文件是“定位入口”，不替代 ExecPlan 证据记录；详细证据仍需回填对应 ExecPlan。
